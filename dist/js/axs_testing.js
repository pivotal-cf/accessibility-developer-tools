 ...
 *
 * Generated from http://github.com/GoogleChrome/accessibility-developer-tools/tree/e82036bfd4d807263faab8bc1be22fd1806fa9ad
 *
...
axs.AuditRule.collectMatchingElements = function(a, b, c, d) {
  axs.AuditRule.collectMatchingElementsInternal(a, b, c, null, d);
};
axs.AuditRule.collectMatchingElementsInternal = function(a, b, c, d, e) {
  if (a.nodeType == Node.ELEMENT_NODE) {
    var f = a;
    if (e) {
      for (var g = 0;g < e.length;g++) {
        if (axs.browserUtils.matchSelector(f, e[g])) {
          return;
        }
      }
    }
  }
  f && b.call(null, f) && c.push(f);
  if (f && (g = d || f.webkitShadowRoot)) {
    axs.AuditRule.collectMatchingElementsInternal(g, b, c, g, e);
    return;
  }
  if (f && "content" == f.localName) {
    for (var h = f.getDistributedNodes(), g = 0;g < h.length;g++) {
      axs.AuditRule.collectMatchingElementsInternal(h[g], b, c, d, e);
    }
  } else {
    if (f && "shadow" == f.localName) {
      if (g = f, d) {
        for (h = g.getDistributedNodes(), g = 0;g < h.length;g++) {
          axs.AuditRule.collectMatchingElementsInternal(h[g], b, c, d, e);
        }
      } else {
        console.warn("ShadowRoot not provided for", f);
      }
    }
    f && "iframe" == f.localName && f.contentDocument && axs.AuditRule.collectMatchingElementsInternal(f.contentDocument, b, c, d, e);
    for (a = a.firstChild;null != a;) {
      axs.AuditRule.collectMatchingElementsInternal(a, b, c, d, e), a = a.nextSibling;
    }
  }
};
axs.AuditRule.prototype.run = function(a) {
  ...
  axs.AuditRule.collectMatchingElements("scope" in a ? a.scope : document, this.relevantElementMatcher_, d, b);
  var e = [];
  if (!d.length) {
